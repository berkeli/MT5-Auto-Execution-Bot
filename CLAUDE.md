...
- **Follow Server TP** (`tp_config.follow_server_tp`, checkbox at the top of Settings → Take-profit, **off by default**): makes the signal service the TP trigger authority. `TPEngine._triggered` ignores `profit_threshold`/`others_pnl` entirely and fires when the signal is in `server_tp_signals` — the auto-TP'd set (`status='profit'` AND `closed_reason != 'manual'`) that `SyncCycle._check_forced_exits` publishes off its existing rev-gated status snapshot, so the feature adds **no Supabase egress**. `_tp_loop` passes the set into `run_cycle` each cycle. Everything downstream of the trigger is unchanged (partial close, trailing, `signal_tp_fired` + pending teardown, weekend profit flatten); trigger rows are tagged `tp_strategy='follow_server'`. Manual `profit` stays on the force-exit path (whole position closed), never routed through TP.
