...
- **v1.6.0 defaults** (data-driven; new installs / "Restore defaults" only — never migrated onto existing users, who keep every value they had): `lot_sizing.mode` defaults to `total_lot`; `lot_sizing.skip_limits_at` (default **0 = off**, N > 0 skips signals carrying ≥ N limits at placement — existing pendings/fills untouched, gated in `sync_cycle` after the unmanaged filter); `partial_close_percent` defaults to **0 = trail the full position** (trailing exits beat fixed TP on real outcome data). These ship only in the bundled `config.example.json`; `migrate_config` leaves `lot_sizing` and `tp_config` *values* alone so an updating user's original settings survive — the sole exception is an additive `setdefault` backfill of new `tp_config.instrument_overrides` keys (e.g. `DE40`/`UK100`/`XTIUSD` and higher-priced stocks), which only adds instruments a user hasn't already tuned and never overwrites an existing override. `POST /api/config/reset` restores the bundled template while preserving license/terminal-path/symbol wiring ("Restore defaults" button in Settings).
