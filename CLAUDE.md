...
- **Friday pendings are keyed on whether the signal is working** (`_CycleContext.row_cancel_blocked`): the late-market reprieve above assumes a ladder can keep working, which stops being true when the market shuts for 48h. So inside `is_weekend_window`, a signal with **no fill** (`signal_id not in ctx.filled_sids`) has its pendings pulled at the placement boundary as before, while one already filled and not yet TP'd keeps its remaining limits to average in before the close. Expired signals need no special case — their limits leave the Supabase active set and `_cancel_stale_pending` takes them, which it can now do during late market because the gate no longer strips those rows out of `ctx.sqlite_pending` first. Crypto stays exempt (it trades the weekend).
